...
桌面 AI 翻译软件（Windows 优先）：划词翻译 · 复制翻译 · 截图翻译 · 邮件助手。
**内置免费翻译引擎，装完即用，无需任何配置**；配置你自己的大模型（任意 OpenAI 兼容接口）可解锁
风格控制（美式 / 英式 / 正式 / 口语 / 学术 / 简洁）、邮件改写与更高翻译质量。

## 功能

| 功能 | 触发方式 | 行为 |
| --- | --- | --- |
| 划词翻译 | 选中文字后**连按两次 Ctrl+C**，或按 `Ctrl+Alt+X`，或点击划词气泡 | 光标下方弹出译文弹窗（可拖动、可钉住） |
| 复制翻译 | 托盘/设置里开启后，复制任意文本 | 自动弹窗翻译（自家复制的译文不会触发） |
| 截图翻译 | `Ctrl+Alt+S` 框选区域 | 本地 OCR 识别 → 翻译，弹窗定位在框选区域**外侧**，不覆盖原文；弹窗内可查看识别原文 |
| 主窗口 | 点击托盘/任务栏图标 | 手动输入翻译 + 邮件助手 + 历史 + 设置 |

快捷键都可在「设置」里改（如 `<ctrl>+<alt>+x`）；截图翻译可单独设定目标语言。
...
开箱即用：默认「自动」引擎——没配大模型就用内置免费翻译（基于公开翻译接口，两端点自动回退），装完即可划词/截图翻译。
