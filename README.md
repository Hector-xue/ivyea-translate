...
**内置免费翻译引擎（DeepL / Google / 腾讯 TranSmart / 必应 并行对冲、自动回退），装完即用，无需任何配置**；
...
| 截图翻译 | `Ctrl+Alt+S` 框选区域 | 文字识别默认用 Windows 系统 OCR（几百毫秒、零下载，与微信截图识别同一量级），不可用时回退本地 RapidOCR 流水线（版面检测后逐段识别、识完一段立刻翻译一段）；弹窗定位在框选区域**外侧**，不覆盖原文；弹窗内可查看识别原文 |
...
「设置 → 外观主题」可在五套皮肤间即点即换。三套带实拍背景照片和对应动效，另有两套纯色主题：
...
| 清绿 | 纯色（品牌淡绿） | 无动效，安静干净 |
| 墨夜 | 纯色（深色） | 无动效，长时间盯着不累 |

- **启用动效**：关掉后只保留静态背景照片，完全不占 CPU；窗口隐藏到托盘时动效本来也会自动停；纯色主题本来就不跑动效。
- **显示主题横幅**：顶部横幅会随机轮播一句诗词 / 古文 / 电影台词并注明出处（内置 127 条），每 25 秒换一句，点一下立刻换下一句；也可一键收起，小窗口下把空间全留给翻译区。
- **前景透明度**：卡片的不透明度可在 55%~100% 之间自由调；调低能看见背景照片，调高看字更省力。输入框和划词弹窗不跟着透——弹窗常年浮在别人家窗口上，透了就看不清。
- 图片类主题的素材都是取自**公有领域 / CC0** 的实拍照片，逐张来源见 [`assets/themes/CREDITS.md`](assets/themes/CREDITS.md)。
...
├── free_engine.py       # 免费引擎（DeepL/Google/TranSmart/必应 对冲回退，纯逻辑可测）
├── translator.py        # prompt 编译（纯函数）+ 翻译线程 + 段落级并发翻译
├── ocr.py               # 识别引擎选择 + RapidOCR 流水线（版面检测 → 逐段识别），行框合并段落（纯函数）
├── ocr_windows.py       # Windows 系统 OCR（Windows.Media.Ocr）：快、零下载，失败回退 RapidOCR
...
Ivyea Translate 一直是免费开源的。如果这个小工具帮你省下了时间，欢迎请作者喝杯咖啡——一杯咖啡就是下个版本的动力。当然，点个 Star，同样是很大的支持。
