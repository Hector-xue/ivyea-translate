...
用户找不到出口就等于把屏幕黏死了。退路——工具条 ✕、Esc、切到别的窗口
（WindowDeactivate，钉住时豁免）。**没有"点一下就关"**（v0.26.3 教训：
正在读译文时手一抖就全没了）。工具条只用按钮不用 QMenu：菜单一弹会触发
WindowDeactivate，把自己关了。
"""
...
class OverlayToolbar(QWidget):
    ...
    def __init__(self, parent: QWidget):
        ...
        for b in (self.btn_copy_res, self.btn_copy_src, self.btn_orig,
                  self.btn_popup, self.btn_pin, self.btn_close):
...
class InPlaceOverlay(QWidget):
    ...
    def set_block_text(self, index: int, text: str) -> None:
        ...
        self.repaint()  # 同步上屏：异步 update 在不抢焦点的置顶窗上偶尔等到下次输入
    ...
    # ---- 交互 ----
    #
    # 没有"点一下就关"（v0.26.3 教训：正在读译文时手一抖就全没了，极易误触）。
    # 关闭途径：工具条 ✕、Esc、切到别的窗口（WindowDeactivate，钉住时豁免）。

    @property
    def is_pinned(self) -> bool:
        return self._toolbar.btn_pin.isChecked()
    ...
    def event(self, ev):
        # 切到别的窗口就收走：置顶覆盖层最烦的是"人都走了它还赖着"。钉住豁免。
        if ev.type() == QEvent.WindowDeactivate and self.isVisible() and not self.is_pinned:
            self.close()
            return True
